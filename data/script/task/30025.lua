...
--15 多少次以后必绿
--////////////////////////////////////////////
function RoleTaskInfo30025:GetTaskIntroText(uid)
	local msg =
		[[
title(收集情报)
body(
blankline()  去国外收集情报，并将获得的情报交给goto(165,7)。
blankline()  每次接取收集情报任务需消耗35体力值，可获取1.5倍经验或丰厚道具奖励。
...
--任务追踪
function RoleTaskInfo30025:GetTaskTraceText(uid)
	local msg = [[<p><n>  目标国：$1</n><event hotkeyid="89" eid="100" cid="0" text="领团魔甲"/><newline/><n> 跳转NPC：</n><goto id="126" mapid="7" /><newline/><n> 去找：</n><goto id="60102" mapid="7" /><newline/><n> 或找：</n><goto id="60103" mapid="6" /></p>]]
	local countryid = s_getTaskVar(uid,30025,11)
...
--任务经验
function RoleTaskInfo30025:GetTaskBaseExp(uid)
	--70*(50+0.1*人物等级^2.2)*颜色倍率*次数及发布加成
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	local base_exp = 70*(50+0.1*level^2.2)
	local color = s_getTaskValue(uid,30025,TaskValue_Color)
	if color == nil then
		color = 0
	end
	local curRound, curRing = GetTaskCurRoundAndRing(uid, 30025)
	local times = curRing
	local timerate = ((times-1)*0.2+1)
	return math.floor(base_exp * self.color_rates[color+1] * timerate)
end

function RoleTaskInfo30025:GetTaskExp(uid)
	local exp = self:GetTaskBaseExp(uid)
	local tiledtimes = 1
	if s_getVar(uid, 30025, TvarIndex_TiledMode) == 1 then
		if s_getVar(uid, 30025, TvarIndex_TiledTimes) == 1 then tiledtimes = 1.5 end
		if s_getVar(uid, 30025, TvarIndex_TiledTimes) == 3 then tiledtimes = 2.5 end
	end
	return math.floor(exp * tiledtimes)
end

--任务描述
function RoleTaskInfo30025:GetSubTaskDescText(uid, taskid, subtaskid)
	local text = nil
	...
	local offline = s_getVar(uid,8,3)*0.1
	text = string.gsub(text,"$OFFLINE",offline)
	return text
end

--检查是否可以接受任务
function RoleTaskInfo30025:CheckAccept(uid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	if process ~= TaskProcess_None then
		return VALUE_FAIL
	end
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	local minLevel = s_getTaskValue(uid,30025,TaskValue_MinLevel)
	local maxLevel = s_getTaskValue(uid,30025,TaskValue_MaxLevel)
	...
	if CheckRoundAndRing(uid, 30025) == VALUE_FAIL then
		return VALUE_FAIL
	end
	return VALUE_OK
end

function RoleTaskInfo30025:CheckAcceptTiled1(uid)
...
function RoleTaskInfo30025:CheckAcceptTiled3(uid)
...
--接受任务对话
function RoleTaskInfo30025:ShowAccept(uid)
	local homeland = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_HOMELAND)
	local country = GetQBTaskTargetCountry(homeland)
	local msg = GetSubTaskAcceptText(uid,30025, 1)
	local call = "RoleTaskInfo30025:AddTask($1,"..country ..")"
	ShowNpcMenuBottomStyle(uid,msg,"确定(3)",call,"离开(3)")
end

function RoleTaskInfo30025:ShowAcceptTiled1_1(uid)
	s_addVar(uid, 30025, TvarIndex_TiledTimes, 1)
	s_addVar(uid, 30025, TvarIndex_TiledMode, 1)
	self:ShowAccept(uid)
end

function RoleTaskInfo30025:ShowAcceptTiled1_2(uid)
	s_addVar(uid, 30025, TvarIndex_TiledTimes, 1)
	s_addVar(uid, 30025, TvarIndex_TiledMode, 2)
	self:ShowAccept(uid)
end

function RoleTaskInfo30025:ShowAcceptTiled3_1(uid)
	s_addVar(uid, 30025, TvarIndex_TiledTimes, 3)
	s_addVar(uid, 30025, TvarIndex_TiledMode, 1)
	self:ShowAccept(uid)
end

function RoleTaskInfo30025:OnAcceptTask(uid)
	local homeland = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_HOMELAND)
	local countryid = s_getCountryVar(homeland,1,15)
	s_addTaskVar(uid,30025,11,countryid)
end

--接受任务
function RoleTaskInfo30025:AddTask(uid,countryid)
	OnOfflineTaskExpEvent(uid,5)
	s_addVar(uid,8,103,s_getCurTime(TIME_ALLDAYS))

	AddRoleSubTask(uid,30025, 1, 1, 0)
	--情报任务刷新次数重置
	s_addTaskVar(uid,30025,15,0)

	--扣体力
	if s_getVar(uid, 30025, TvarIndex_TiledTimes) == 1 then
		s_subValue(SCENE_ENTRY_PLAYER, uid, VALUE_TYPE_USABLE_TILED_POINT, 35)
	elseif s_getVar(uid, 30025, TvarIndex_TiledTimes) == 3 then
...
--是否显示交付任务
function RoleTaskInfo30025:CheckFinish(uid,minlevel,maxlevel)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	if process == TaskProcess_None then
		return VALUE_FAIL
	end
...
--显示完成任务对话
function RoleTaskInfo30025:ShowFinish(uid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	...
	local msg = GetSubTaskFinishText(uid,30025,1)
	local call = "RoleTaskInfo30025:FinishTask($1)"
	ShowNpcMenuBottomStyle(uid,msg,"完成(3)",call,"离开(3)")
end

--避免CommonTaskReward
function RoleTaskInfo30025:TaskReward(uid, taskid, subtaskid)
end

--交付任务
function RoleTaskInfo30025:FinishTask(uid)
	s_removebuff(SCENE_ENTRY_PLAYER,uid,94)
	--获得经验
	local exp = GetTaskExp(uid,30025)
	...
	if offline ~= 0 then
		...
		s_refreshExp(uid,exp,ExpType_OptionalTask,30025)
	...
	if worldexp ~= 0 then
		s_refreshExp(uid,worldexp,ExpType_OptionalTask,30025)
	...
	if step == EventStep_Run and s_getTaskVar(uid,30025,12) == 1 then
		local extra_exp = math.floor(exp*0.5)
		s_refreshExp(uid,extra_exp,ExpType_OptionalTask,30025)
	...
	if step1 == EventStep_Run and (step ~= EventStep_Run or s_getTaskVar(uid,30025,12) ~= 1) then
		local extra_exp = math.floor(exp*0.5)
		s_refreshExp(uid,extra_exp,ExpType_OptionalTask,30025)
	...
	Dealtaskexpitem(uid)
	--体力宝箱
	if s_getVar(uid, 30025, TvarIndex_TiledMode) == 2 then
		s_addItem(uid,29303,0,1,0,"1-1","RoleTaskInfo30025")
	end
	...
	local medal = s_getTaskValue(uid,30025,TaskValue_ExpMedal)
	...
	--奥运活动
	local process = s_getTaskValue(uid,30327,TaskValue_Process)
	if process == TaskProcess_Doing then
		AddRoleTaskOperate(uid,30327)
	end
...
--完成次数
function RoleTaskInfo30025:OnTaskDailyChanged(uid)
	RefreshTaskDailyInfo(uid, 30082)
end

--NPC菜单
--AppendMenuToNpc(165,"收集情报(2)","RoleTaskInfo30025:ShowAccept($1)","RoleTaskInfo30025:CheckAccept($1)")
AppendMenuToNpc(165,"收集情报- 35体力（1.5倍经验）(2)","RoleTaskInfo30025:ShowAcceptTiled1_1($1)","RoleTaskInfo30025:CheckAcceptTiled1($1)")
AppendMenuToNpc(165,"收集情报- 35体力（1倍经验+宝箱）(2)","RoleTaskInfo30025:ShowAcceptTiled1_2($1)","RoleTaskInfo30025:CheckAcceptTiled1($1)")
AppendMenuToNpc(165,"收集情报- 105体力（2.5倍经验）(2)","RoleTaskInfo30025:ShowAcceptTiled3_1($1)","RoleTaskInfo30025:CheckAcceptTiled3($1)")
AppendMenuToNpc(165,"收集情报(2)","RoleTaskInfo30025:ShowFinish($1)","RoleTaskInfo30025:CheckFinish($1)")

--//====================================================================================
--// 中间对话NPC
--//====================================================================================
--检查NPC对当前任务是否有效
function RoleTaskInfo30025:CheckEffectNpc(uid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	if process ~= TaskProcess_Doing then
		return VALUE_FAIL
	end
	local target_country = s_getTaskVar(uid,30025,11)
...
--请求获得情报(进度条)
function RoleTaskInfo30025:RequestQingBao(uid,npcid)
	local callback = "RoleTaskInfo30025:GetQingbao($1,"..npcid..")"
...
function RoleTaskInfo30025:GetQingbao(uid,npcid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	if process == TaskProcess_None then
		return
	end
	...
	s_addTaskVar(uid,30025,13,now)
	s_addTaskVar(uid,30025,14,now)

	AddRoleTaskOperate(uid,30025,"获得情报")
	local cold = RequestVerifyQingBao30025(uid,npcid)--给情报加剩余时间BUFF
	...
	--刷新次数决定情报颜色
	if s_getTaskVar(uid,30025,15) >= 15 then
		rand = math.random(1,100)
		if rand <= 97 then
	...
	RoleTaskInfo30025:RefreshTaskColor(uid,color)
	local names = {"白色","蓝色","黄色","绿色","紫色"}
	s_sysInfo(uid,string.format("居然获得了%s情报",names[color+1]))
	--给刷情报计算次数
	s_addTaskVar(uid,30025,15,s_getTaskVar(uid,30025,15)+1)
end

--检查是否显示获得情报接口
function RoleTaskInfo30025:ShowQingBao(uid)
	return RoleTaskInfo30025:CheckEffectNpc(uid)
end

--刷新任务颜色
function RoleTaskInfo30025:RefreshTaskColor(uid,color)
	s_setTaskValue(uid,30025,TaskValue_Color,color)
	--经验修改，重置任务描述
	if color ~= oldcolor then
		s_addTaskVar(uid,30025,MvarIndex_Exp,0)
		local text = GetSubTaskDescText(uid, 30025, 1)
		s_setTaskValue(uid,30025,TaskValue_Desc,text,1)
	end
	return VALUE_OK
end

--检查是否显示刷新任务颜色
function RoleTaskInfo30025:ShowRefresh60102(uid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	...
	--[[2分钟冷却
	local last = s_getTaskVar(uid,30025,13)
	local now = s_getCurTime(TIME_ALLSECONDS)
...
--检查是否显示刷新任务颜色
function RoleTaskInfo30025:ShowRefresh60103(uid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	if process ~= TaskProcess_CanDone then
		return VALUE_FAIL
	end
...
function RequestVerifyQingBao30025(uid,npcid)
	local cold = 45
	local homeland = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_HOMELAND)
	local countrystate = s_getCountryState(homeland)
	if countrystate == CountryStateWeak then
...
--请求核对情报
function RoleTaskInfo30025:RequestVerifyQingBao(uid,npcid)
	local cold = RequestVerifyQingBao30025(uid,npcid)
	local last = s_getTaskVar(uid,30025,13)
	local now = s_getCurTime(TIME_ALLSECONDS)
	if now <= last or (now - last <= cold) then
		local msg = string.format("  请%u秒后再来核对,核对15次后必定为绿色。",last + cold - now + 1)
...
--玩家死亡，丢失情报
function RoleTaskInfo30025:OnDead(uid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	if process == TaskProcess_CanDone then
	end
end
...
--玩家删除任务
function RoleTaskInfo30025:OnDeleteTask(uid)
	s_removebuff(SCENE_ENTRY_PLAYER,uid,84)
	s_removebuff(SCENE_ENTRY_PLAYER,uid,90)
	s_removebuff(SCENE_ENTRY_PLAYER,uid,91)
	s_removebuff(SCENE_ENTRY_PLAYER,uid,92)
...
--请求获得情报(进度条)
function RoleTaskInfo30025:RequestQingBaoSafe(uid)
	local callback = "RoleTaskInfo30025:GetQingbaoSafe($1)"
	s_showbar(uid,10000,callback,0,1)
end

function RoleTaskInfo30025:GetQingbaoSafe(uid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	if process == TaskProcess_None then
		return
	end
	...
	RoleTaskInfo30025:RefreshTaskColor(uid,color)
	local names = {"白色","蓝色","黄色","绿色","紫色"}
	s_sysInfo(uid,string.format("获得了%s情报",names[color+1]))
end

--检查是否显示获得情报接口
function RoleTaskInfo30025:ShowQingBaoSafe(uid)
	local process = s_getTaskValue(uid,30025,TaskValue_Process)
	if process ~= TaskProcess_Doing then
		return VALUE_FAIL
	end
	local homeland = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_HOMELAND)
	local scene_country = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_CURRENT_COUNTRY)
	if homeland ~= scene_country then
		return VALUE_FAIL
	end
	return VALUE_OK
end
...
--邦德
AppendMenuToNpc(60102,"情报任务(2)","RoleTaskInfo30025:RequestQingBao($1,60102)","RoleTaskInfo30025:ShowQingBao($1)")
AppendMenuToNpc(60102,"核对情报任务品质(2)","RoleTaskInfo30025:RequestVerifyQingBao($1,60102)","RoleTaskInfo30025:ShowRefresh60102($1)")
--辛西娅
AppendMenuToNpc(60103,"情报任务(2)","RoleTaskInfo30025:RequestQingBao($1,60103)","RoleTaskInfo30025:ShowQingBao($1)")
AppendMenuToNpc(60103,"核对情报任务品质(2)","RoleTaskInfo30025:RequestVerifyQingBao($1,60103)","RoleTaskInfo30025:ShowRefresh60103($1)")
--国内安全情报员
AppendMenuToNpc(60187,"获取一份传言（白色情报）(2)","RoleTaskInfo30025:RequestQingBaoSafe($1)","RoleTaskInfo30025:ShowQingBaoSafe($1)")
