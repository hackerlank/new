...
--任务简介
function RoleTaskInfo30023:GetTaskIntroText(uid)
	local msg =
		[[
title(前线速递)
body(
blankline()   在goto(60559,5)处接取帝国军方的补给车，并将它押送到指定的目的地。
blankline()   每次接取前线速递任务需消耗45体力值，可获取1.5倍经验或丰厚道具奖励。
blankline()   每天可完成2次，hot(周日可额外完成1次)。周日完成任务所获得的数字，将适在周六抽奖时使用。
blankline()   exp(海量)
)
]]
...
--任务经验
function RoleTaskInfo30023:GetTaskBaseExp(uid)
	...
	local curRound, curRing = GetTaskCurRoundAndRing(uid, 30023)
	local timerate = ((curRing-1)*0.2+1)
	local color = s_getTaskValue(uid,30023,TaskValue_Color)
	if color == nil then color = 0 end
	local pass_color = s_getTaskVar(uid,30023,15)
	local pass = 1
	...
	--接任务经验显示正确
	if s_getTaskValue(uid,30023,TaskValue_Process) == TaskProcess_None then
		--local itemid = RoleTaskInfo30023:CheckPutPass(uid)
		local itemid = s_getVar(uid,30023,200)--s_getTaskVar(uid,30023,16)
		if itemid ~= VALUE_FAIL then
			local quality = s_getItemValue(uid,itemid,Item_Attribute_Quality)
			exp = exp * self.pass_rates[quality+1]
		end
		local capsuleid = s_getVar(uid,30023,201)
		if capsuleid ~= 0 then
			local color = RoleTaskInfo30023:GetColorByCapsule(uid,capsuleid)
...
function RoleTaskInfo30023:GetTaskExp(uid)
	local exp = self:GetTaskBaseExp(uid)
	local tiledtimes = 1
	if s_getVar(uid, 30023, TvarIndex_TiledMode) == 1 then
		if s_getVar(uid, 30023, TvarIndex_TiledTimes) == 1 then tiledtimes = 1.5 end
	end
...
--掉落的货物对应的经验
function RoleTaskInfo30023:GetTaskExp_Drop(uid,color)
	--75*短期化系数*(30+人物等级^1.5)*颜色加成*次数加成*通行证加成
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	local expmode = 0
	for i = 1, table.getn(self.exp_mode) do
...
--获得交付目标NPC
function RoleTaskInfo30023:GetTargetNpc(uid)
	local npc = s_getTaskVar(uid,30023,12)
	local mapid = s_getTaskVar(uid,30023,13)
	...
	s_addTaskVar(uid,30023,12,target)
	s_addTaskVar(uid,30023,13,map)
	return target,map
end

--任务追踪
function RoleTaskInfo30023:GetTaskTraceText(uid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	if level >= 61 then
		if s_getTaskVar(uid,30023,17) == 0 and s_getTaskVar(uid,30023,21) == 0 then
			local msg = [[body( 17:00-22:00可投保：goto(226,5)         物资装填：goto(192,5)         goto(227,7)         任务回复：goto(%u,%u))]]
			local npcid,mapid = RoleTaskInfo30023:GetTargetNpc(uid)
			msg = string.format(msg,npcid,mapid)
			return parseToDesc(msg)
		elseif s_getTaskVar(uid,30023,17) == 1 and s_getTaskVar(uid,30023,21) == 0 then
			local msg = [[body( 17:00-22:00可投保：goto(226,5)         物资装填：goto(227,7)         任务回复：goto(%u,%u))]]
			local npcid,mapid = RoleTaskInfo30023:GetTargetNpc(uid)
			msg = string.format(msg,npcid,mapid)
			return parseToDesc(msg)
		end
...
--任务描述
function RoleTaskInfo30023:GetSubTaskDescText(uid, taskid, subtaskid)
	local target,map = RoleTaskInfo30023:GetTargetNpc(uid)
	local text = nil
	...
	local foregift = RoleTaskInfo30023:GetTaskForegift(uid)
...
--押金
function RoleTaskInfo30023:GetTaskForegift(uid)
	local foregift = s_getTaskVar(uid,30023,14)
	if foregift == 0 then
		foregift = 20
		local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	...
	if s_getTaskVar(uid,30023,11) ~= 1 then
		return foregift
	else
...
--每天次数
function RoleTaskInfo30023:RefreshTaskRing(uid)
	--速递日可完成4次
...
--获得补给车ID
function RoleTaskInfo30023:GetGuardID(uid,color)
...
--检查是否可以接受任务
function RoleTaskInfo30023:CheckAccept(uid)
	RoleTaskInfo30023:RefreshTaskRing(uid)
	...
	local pretask = s_getTaskValue(uid,30023,TaskValue_PreTask)
	...
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	local minLevel = s_getTaskValue(uid,30023,TaskValue_MinLevel)
	local maxLevel = s_getTaskValue(uid,30023,TaskValue_MaxLevel)
	...
	if CheckRoundAndRing(uid, 30023) == VALUE_FAIL then
		return VALUE_FAIL
	end
	return VALUE_OK
end

function RoleTaskInfo30023:CheckAcceptTiled1(uid)
	return self:CheckAccept(uid)
end

--显示通行证和胶囊界面
function RoleTaskInfo30023:ShowAcceptWithItem(uid)
	...
	msg = parseToTalk(msg)
	ShowTaskPack2(uid,"运镖",msg,"确定","请放入通行证:","请放入储色胶囊:","31145","31123:31122:31121","RoleTaskInfo30023:ShowAccept($1,$2,$3)")
end

function RoleTaskInfo30023:ShowAcceptWithItemTiled1_1(uid)
	if s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_USABLE_TILED_POINT) < 45 then
		s_sysInfo(uid,"体力值不足45点，无法接取任务。")
		return VALUE_FAIL
	end
	s_addVar(uid, 30023, TvarIndex_TiledTimes, 1)
	s_addVar(uid, 30023, TvarIndex_TiledMode, 1)
	self:ShowAcceptWithItem(uid)
end

function RoleTaskInfo30023:ShowAcceptWithItemTiled1_2(uid)
	if s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_USABLE_TILED_POINT) < 45 then
		s_sysInfo(uid,"体力值不足45点，无法接取任务。")
		return VALUE_FAIL
	end
	s_addVar(uid, 30023, TvarIndex_TiledTimes, 1)
	s_addVar(uid, 30023, TvarIndex_TiledMode, 2)
	self:ShowAcceptWithItem(uid)
end

--接受任务对话
function RoleTaskInfo30023:ShowAccept(uid,passid,capsuleid)
	if passid == nil then passid = 0 end
	if passid == 0 then
		s_addVar(uid,30023,200,0)
	else
		s_addVar(uid,30023,200,passid)
	end
	if capsuleid == 0 then
		s_addVar(uid,30023,201,0)
	else
		s_addVar(uid,30023,201,capsuleid)
	end
	local msg = GetSubTaskAcceptText(uid,30023,1)
	local foregift = RoleTaskInfo30023:GetTaskForegift(uid)
	msg = string.gsub(msg,"$FOREGIFT",foregift)
	local call = string.format("RoleTaskInfo30023:StartFreshColor($1,%u,%u)",passid,capsuleid)
	--有颜色胶囊的不再走摇颜色
	if capsuleid > 0 then
		call = string.format("RoleTaskInfo30023:AddTaskWithCapsule($1,%u,%u)",passid,capsuleid)
	end
	ShowNpcMenuBottomStyle(uid,msg,"接受(3)",call,"离开(3)")
end

--开始摇颜色
function RoleTaskInfo30023:StartFreshColor(uid,passid,capsuleid)
	...
	RoleTaskInfo30023:AddTask(uid,{passid,capsuleid},{color,0})
end

--接受任务（颜色胶囊）
function RoleTaskInfo30023:AddTaskWithCapsule(uid,passid,capsuleid)
	local param = {passid,capsuleid}
	local param_ext = {0, 0}
	RoleTaskInfo30023:AddTask(uid,param,param_ext)
end

--接受任务
function RoleTaskInfo30023:AddTask(uid,param,param_ext)
	...
	if capsuleid ~= 0 and capsuleid ~= nil then
		...
		if RoleTaskInfo30023:GetColorByCapsule(uid,capsuleid) == TaskColor_White then
	...
	if passid ~= 0 and passid ~= nil then
		...
		if RoleTaskInfo30023:CheckPassValid(uid,baseid,passid) ~= VALUE_OK then
			s_sysInfo(uid,"该通行证不可使用")
			return VALUE_FAIL
		end
	end

	if RoleTaskInfo30023:CheckAccept(uid) ~= VALUE_OK then
		return VALUE_FAIL
	end
	...
	if capsuleid ~= 0 and capsuleid ~= nil then
		...
		color = RoleTaskInfo30023:GetColorByCapsule(uid,capsuleid)
	end
	local guardid = RoleTaskInfo30023:GetGuardID(uid,color)
	local endnpc = RoleTaskInfo30023:GetTargetNpc(uid)
	local dropexp = RoleTaskInfo30023:GetTaskExp_Drop(uid,color)
	local npc = s_summonGuard(uid,guardid,30023,endnpc,foregift,dropexp)
	...
	OnOfflineTaskExpEvent(uid,2)
	s_addVar(uid,8,111,s_getCurTime(TIME_ALLDAYS))

	AddRoleSubTask(uid,30023,1,1,1,color)
	s_addTaskVar(uid,30023,14,foregift)
	s_addTaskVar(uid,30023,MvarIndex_Hourglass,0)
	...
	if capsuleid ~= nil and capsuleid ~= 0 then
		s_deleteItemByThisID(uid,capsuleid,1,"RoleTaskInfo30023")
		s_addVar(uid,100,6,today)
	end

	--扣体力
	if s_getVar(uid, 30023, TvarIndex_TiledTimes) == 1 then
	...
	if passid ~= VALUE_FAIL and passid ~= nil then
		...
		if s_deleteItemByThisID(uid,thisid,1,"RoleTaskInfo30023") == VALUE_OK then
			s_addTaskVar(uid,30023,15,pass_color+1)
			--s_addTaskVar(uid,30023,16,passid)
			--重新计算任务经验(通行证影响经验)
			s_addTaskVar(uid,30023,MvarIndex_Exp,0)
			local text = GetSubTaskDescText(uid,30023,1)
			s_setTaskValue(uid,30023,TaskValue_Desc,text,1)
		end
	end
	--国家发布时间，做标记
	local homeland = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_HOMELAND)
	local step = s_getEvent(Relation_Country,homeland,CountryEv_GuoYun)
	if step == EventStep_Run then
		s_addTaskVar(uid,30023,20,1)
	end
	return VALUE_OK
end

--是否显示交付任务
function RoleTaskInfo30023:CheckShowFinish(uid,npcid)
	RoleTaskInfo30023:RefreshTaskRing(uid)
	local process = s_getTaskValue(uid,30023,TaskValue_Process)
	if process == TaskProcess_None then
		return VALUE_FAIL
	...
	local target = s_getTaskVar(uid,30023,12)
...
--官员托运的检查
function RoleTaskInfo30023:CheckShowFinish2(uid,npcid)
	RoleTaskInfo30023:RefreshTaskRing(uid)
	local process = s_getTaskValue(uid,30023,TaskValue_Process)
	if process == TaskProcess_None then
		return VALUE_FAIL
...
--官员托运完成任务
function RoleTaskInfo30023:ShowFinish2(uid)
	s_changeGuardTargetNpc(uid,30023,218)
	RoleTaskInfo30023:ShowFinish(uid)
end
--显示完成任务对话
function RoleTaskInfo30023:ShowFinish(uid)
	--没有补给车，提示玩家放弃任务
	local guard = s_getGuardID(uid)
	if guard  == 0 then
		s_sysInfo(uid,"补给车丢失,任务失败，请删除任务后重新接取 ")
		return
	end
	s_resetTrafficType(uid)
	local msg = GetSubTaskFinishText(uid,30023,1)
	local call = "RoleTaskInfo30023:FinishTask($1)"
	ShowNpcMenuBottomStyle(uid,msg,"完成(3)",call,"离开(3)")
end

--避免CommonTaskReward
function RoleTaskInfo30023:TaskReward(uid, taskid, subtaskid)
end

--交付任务
function RoleTaskInfo30023:FinishTask(uid)
	if s_commitGuard(uid,30023) ~= VALUE_OK then
	...
	if color == 0 then
		s_debug(username.."完成白色高等级前线速递")
	elseif color == 1 then
		s_debug(username.."完成蓝色高等级前线速递")
	elseif color == 2 then
		s_debug(username.."完成黄色高等级前线速递")
	elseif color == 3 then
		s_debug(username.."完成绿色高等级前线速递")
	elseif color == 4 then
		s_debug(username.."完成紫色高等级前线速递")
	end
	--返还押金
	local foregift = RoleTaskInfo30023:GetTaskForegift(uid)

	local exp = GetTaskExp(uid,30023) --获得经验
	...
	if offline ~= 0 then
		...
		s_refreshExp(uid,exp,ExpType_OptionalTask,30023)
	...
	if worldexp ~= 0 then
		s_refreshExp(uid,worldexp,ExpType_OptionalTask,30023)
	...
	if step == EventStep_Run and s_getTaskVar(uid,30023,20) == 1 then
		local extra_exp = math.floor(exp*0.5)
		s_refreshExp(uid,extra_exp,ExpType_OptionalTask,30023)
	...
	if step1 == EventStep_Run and (step ~= EventStep_Run or s_getTaskVar(uid,30023,20) ~= 1) then
		local extra_exp = math.floor(exp*0.5)
		s_refreshExp(uid,extra_exp,ExpType_OptionalTask,30023)
	...
	if s_getEscort(uid) ~= 0 then
		...
		s_sysInfo(uid,"投保佣兵公会，运输获得额外经验"..extra_exp)
	end
	--清除数据
	s_clearEscort(uid)
	...
	if foregift == 0 then
		...
		if s_getTaskVar(uid,30023,18) == 0 then
			local color = s_getTaskValue(uid,30023,TaskValue_Color)
			if color == 0 then
	...
	Dealtaskexpitem(uid)
	--体力宝箱
	if s_getVar(uid, 30023, TvarIndex_TiledMode) == 2 then
		s_addItem(uid,29303,0,1,0,"1-1","RoleTaskInfo30023")
	end
	...
	s_addTaskVar(uid,30023,17,0)
	s_addTaskVar(uid,30023,21,0)
	s_addTaskVar(uid,30023,22,0)
	s_addTaskVar(uid,30023,18,0)
	FinishRoleTask(uid,30023)
	...
	--RoleTaskInfo1292:AddOperate(uid,1)
	...
	--]]
	--元宵情人节活动
	--[[if month == 2 and (mday >= 13 and mday <= 17) then
	s_addItem(uid,55258,3,1,0,"1-1","开门利是")
	end
...
--获得对应的通行证
function RoleTaskInfo30023:CheckPassValid(uid,baseid,thisid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
...
--获得胶囊对应的补给车颜色
function RoleTaskInfo30023:GetColorByCapsule(uid,itemid)
...
function RoleTaskInfo30023:OnDeleteTask(uid)
	s_clearEscort(uid)
	s_killGuard(uid)
	s_removebuff(SCENE_ENTRY_PLAYER,uid,21)
end
...
--完成次数
function RoleTaskInfo30023:OnTaskDailyChanged(uid)
	RefreshTaskDailyInfo(uid, 30063)
end


--NPC菜单
--AppendMenuToNpc(60559,"前线速递(2)","RoleTaskInfo30023:ShowAcceptWithItem($1)","RoleTaskInfo30023:CheckAccept($1)")
AppendMenuToNpc(60559,"前线速递- 45体力（1.5倍经验）(2)","RoleTaskInfo30023:ShowAcceptWithItemTiled1_1($1)","RoleTaskInfo30023:CheckAcceptTiled1($1)")
AppendMenuToNpc(60559,"前线速递- 45体力（1倍经验+宝箱）(2)","RoleTaskInfo30023:ShowAcceptWithItemTiled1_2($1)","RoleTaskInfo30023:CheckAcceptTiled1($1)")

--60之后交付
AppendMenuToNpc(169,"前线速递(2)","RoleTaskInfo30023:ShowFinish($1)","RoleTaskInfo30023:CheckShowFinish($1,169)")
AppendMenuToNpc(218,"官员托运(2)","RoleTaskInfo30023:ShowFinish2($1)","RoleTaskInfo30023:CheckShowFinish2($1)")
