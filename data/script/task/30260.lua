...
--玩家变量
--30260-11 任务目标阶段
function RoleTaskInfo30260:GetTaskStage(uid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	local stageid = s_getVar(uid,30260,11)
	if stageid == 0 then
...
--任务追踪
function RoleTaskInfo30260:GetTaskTraceText(uid)
	local stageid = RoleTaskInfo30260:GetTaskStage(uid)
...
--任务描述
function RoleTaskInfo30260:GetSubTaskDescText(uid,taskid,subtaskid)
	local text = [[title(制作生存训练卷轴)
	body(    杀死%s后向goto(60000,3)回复。newline()   hot(    （与高于自身等级的其他玩家组队，击败高等级的生存训练所需怪物，依然可以完成任务）)
...
--检查是否可以接受任务
function RoleTaskInfo30260:CheckAccept(uid)
	...
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	local minLevel = s_getTaskValue(uid,30260,TaskValue_MinLevel)
	local maxLevel = s_getTaskValue(uid,30260,TaskValue_MaxLevel)
	...
	if CheckRoundAndRing(uid, 30260) == VALUE_FAIL then
		return VALUE_FAIL
	end
	return VALUE_OK
end

function RoleTaskInfo30260:ShowAccept(uid)
	local msg = GetSubTaskAcceptText(uid,30260,1)
	local addcall = "RoleTaskInfo30260:AddTask($1)"
	ShowNpcMenuBottomStyle(uid,msg,"接受(3)",addcall,"离开(3)")
end

function RoleTaskInfo30260:GetSubTaskAcceptText(uid,taskid,subtaskid)
	--确定收集内容
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	local stageid = RoleTaskInfo30260:GetTaskStage(uid)
...
--添加任务
function RoleTaskInfo30260:AddTask(uid)
	local stageid = RoleTaskInfo30260:GetTaskStage(uid)
	s_addVar(uid,30260,11,stageid)

	OnOfflineTaskExpEvent(uid,3)
	s_addVar(uid,8,102,s_getCurTime(TIME_ALLDAYS))

	local state = self.content[stageid][4]
	AddRoleSubTask(uid,30260,1,state,0)
end

--删除任务
function RoleTaskInfo30260:OnDeleteTask(uid)
	s_addVar(uid,30260,11,0)
end

--检查是否显示完成菜单
function RoleTaskInfo30260:CheckFinish(uid)
	local process = s_getTaskValue(uid,30260,TaskValue_Process)
	if process ~= TaskProcess_None then
		return VALUE_OK
	end
...
--显示交付任务菜单
function RoleTaskInfo30260:ShowFinish(uid)
	local process = s_getTaskValue(uid,30260,TaskValue_Process)
	if process == TaskProcess_Doing then
		local doing = s_getTaskValue(uid,30260,TaskValue_PromptText)
		doing = trim(doing)
		ShowNpcMenuBottomStyle(uid,doing,"离开(5)")
	elseif process == TaskProcess_CanDone then
		local msg = GetSubTaskFinishText(uid,30260,1)
		local str = "RoleTaskInfo30260:FinishTask($1)"
		ShowNpcMenuBottomStyle(uid,msg,"完成(3)",str,"离开(3)")
	end
end

--交付任务
function RoleTaskInfo30260:FinishTask(uid)
	s_addItem(uid,31390,0,1,0,"","RoleTaskInfo30260:FinishTask")
	s_addVar(uid,30260,11,0)
	FinishRoleTask(uid,30260)
end
--杀怪回调
function RoleTaskInfo30260:OnMonsterDead(uid,npcid)
	local process = s_getTaskValue(uid,30260,TaskValue_Process)
	if process ~= TaskProcess_Doing then
		return
	end
	...
	for index = 1,table.getn(self.content) do
		...
		if check == VALUE_OK and target == npcid then
			AddRoleTaskOperate(uid,30260)
			return
		end
	end
end
--注册怪物回调
function RoleTaskInfo30260:RegisterMonsterCall()
	for i = 1, table.getn(self.content) do
		local call = string.format("RoleTaskInfo30260:OnMonsterDead($1,%d)",self.content[i][5])
		RegisterNpcDeadEvent(self.content[i][5],call)
	end
end
RoleTaskInfo30260:RegisterMonsterCall()

--未接取状态说明
function RoleTaskInfo30260:GetTaskIntroText(uid)
	local msg =
		[[
title(生存训练)
body(
...
--NPC菜单
AppendMenuToNpc(60000,"制作生存训练卷轴(2)","RoleTaskInfo30260:ShowAccept($1)","RoleTaskInfo30260:CheckAccept($1)")
AppendMenuToNpc(60000,"制作生存训练卷轴[完成](2)","RoleTaskInfo30260:ShowFinish($1)","RoleTaskInfo30260:CheckFinish($1)")
