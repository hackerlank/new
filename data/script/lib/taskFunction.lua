...
s_sysMail(uid, paramlist[1], paramlist[2], paramlist[3], paramlist[4], paramlist[5])
end
...
for i = 1,table.getn(list), 2 do
	if list[i] == SCENE_ENTRY_PLAYER and list[i+1] ~= uid then
		s_sysMail(list[i+1], paramlist[2], paramlist[3], paramlist[4], paramlist[5], paramlist[6])
	end
end
end
...
function s_taskTalk(taskid, subtaskid, paramlist, uid)
	s_debug("[000-taskFunction:1285]任务:taskid:%d,subtaskid:%d",taskid, subtaskid)
