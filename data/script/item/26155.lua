--超级生命精华-超
function OnUseItemEvent_26155(userid,thisid,targetid)
	...
	local add = GetCureHp(userid,15000)
	if add + hp > maxhp then
		add = maxhp - hp
	end
	local count = s_getItemValue(userid,thisid,Item_Attribute_UseCount)
	if count == 159 then
		if s_deleteItemByBaseID(userid,26155,1,"OnUseItemEvent_26155") ~= VALUE_OK then
			return VALUE_FAIL
		end
