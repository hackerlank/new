function OnUseItemEvent_53100(uid,thisid)
	if s_checkItem(uid,53100,10) ~= VALUE_OK then
		s_sysInfo(uid,"您的符文碎片不够噢！每次合成符文需要10块符文碎片。",ChatPos_Sys)
		return VALUE_FAIL
	end
	local msg = "你是否同意花费500绑银合成符文碎片呢？"
	local buttons = "确定|3|Enter|取消|4|Esc"
	local call = "CallBack_OnUseItemEvent_53100($1,$2," .. thisid .. ")"
	s_registerMsgBox(uid,call,buttons,msg,300)
	return VALUE_OK
end

function CallBack_OnUseItemEvent_53100(uid,result,thisid)
	...
	if s_removeMoney(uid,MoneyType_MoneyTicket,500,"OnUseItemEvent_53100") ~= VALUE_OK then
		s_sysInfo(uid,"需要500绑银哦！")
		return VALUE_FAIL
	end
	s_addItem(uid,53200,3,1,0,"1-"..needbind,"符文合成")
	return VALUE_OK
end
