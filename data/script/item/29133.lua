--叹息之墙（火焰）
function OnUseItemEvent_29133(uid,thisid)
	if s_getMedal(uid) ~= MedalType_Fire then
		return VALUE_FAIL
	end
	local curday = s_getCurTime(TIME_ALLDAYS)
	if s_deleteItemByThisID(uid,thisid,1,"使用道具叹息之墙（火焰）") == VALUE_OK then
		...
		for i = 1,table.getn(list), 2 do
			if list[i] == SCENE_ENTRY_PLAYER then
				...
				if homeland == country then
					if s_getbufflevel(list[i],list[i+1],31059) ~= 0 then
						s_removebuff(list[i],list[i+1],31059)
					end
					s_addbuff(list[i],list[i+1],31059,25,300)
				end
			end
		end
