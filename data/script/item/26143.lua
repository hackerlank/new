--初级生命精华大
function OnUseItemEvent_26143(userid,thisid,targetid)
	...
	local add = GetCureHp(userid,2000)
	if add + hp > maxhp then
		add = maxhp - hp
	end
	local count = s_getItemValue(userid,thisid,Item_Attribute_UseCount)
	if count == 79 then
		if s_deleteItemByBaseID(userid,26143,1,"OnUseItemEvent_26143") ~= VALUE_OK then
			return VALUE_FAIL
		end
