--高级生命精华超
function OnUseItemEvent_26151(userid,thisid,targetid)
	...
	local add = GetCureHp(userid,10000)
	if add + hp > maxhp then
		add = maxhp - hp
	end
	local count = s_getItemValue(userid,thisid,Item_Attribute_UseCount)
	if count == 319 then
		if s_deleteItemByBaseID(userid,26151,1,"OnUseItemEvent_26151") ~= VALUE_OK then
			return VALUE_FAIL
		end
