--舞会门票(中)
function OnUseItemEvent_29066(uid,thisid)
	...
	if s_getbufflevel(SCENE_ENTRY_PLAYER,uid,57) ~= 0 or s_getbufflevel(SCENE_ENTRY_PLAYER,uid,126) ~= 0 then
	...
	if s_deleteItemByThisID(uid,thisid,1,"使用舞票中") ~= VALUE_OK then
		return VALUE_FAIL
	end
	if bufflevel > 0 then
	...
	local process = s_getTaskValue(uid,30620,TaskValue_Process)
	if process == TaskProcess_Doing then
		AddRoleTaskOperate(uid,30620)
	end
	--命运女神的垂青（5次）
	...
	local process = s_getTaskValue(uid,30621,TaskValue_Process)
	if process == TaskProcess_Doing then
		AddRoleTaskOperate(uid,30621)
	end
	--中秋活动
