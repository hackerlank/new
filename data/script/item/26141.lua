--初级生命精华小
function OnUseItemEvent_26141(userid,thisid,targetid)
	...
	local add = GetCureHp(userid,800)
	if add + hp > maxhp then
		add = maxhp - hp
	end
	local count = s_getItemValue(userid,thisid,Item_Attribute_UseCount)
	if count == 19 then
		if s_deleteItemByBaseID(userid,26141,1,"OnUseItemEvent_26141") ~= VALUE_OK then
			return VALUE_FAIL
		end
