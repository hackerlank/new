--单一技能遗忘手册
function OnUseItemEvent_26019(userid,thisid,skillid)
	...
	elseif s_getcold(userid,skillid) ~= 0 then
		s_sysInfo(userid,"处于冷却中的技能不可遗忘！",ChatPos_Sys + ChatPos_Important)
		return VALUE_FAIL
	end

	if s_forgetskill(SCENE_ENTRY_PLAYER,userid,skillid,10000) == VALUE_OK then
		s_deleteItemByThisID(userid,thisid,1,"OnUseItemEvent_26019")
		return VALUE_OK
	end
	return VALUE_FAIL
end
