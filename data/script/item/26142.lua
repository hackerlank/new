--初级生命精华中
function OnUseItemEvent_26142(userid,thisid,targetid)
	...
	local add = GetCureHp(userid,1200)
	if add + hp > maxhp then
		add = maxhp - hp
	end
	local count = s_getItemValue(userid,thisid,Item_Attribute_UseCount)
	if count == 39 then
		if s_deleteItemByBaseID(userid,26142,1,"OnUseItemEvent_26142") ~= VALUE_OK then
			return VALUE_FAIL
		end
