--化减孢子
function OnUseItemEvent_43017(uid,thisid,targetid)
	...
	local jingshen = s_getItemValue(uid,targetid,Item_Attribute_Demon_Jingshen)
	if jingshen < demonlevel then
		s_sysInfo(uid,"当前属性点值必须高于宠物等级")
		return VALUE_FAIL
	end
	if s_deleteItemByThisID(uid,thisid,1,"OnUseItemEvent_43017") ~= VALUE_OK then
		return VALUE_FAIL
	end
	s_setItemValue(uid,targetid,Item_Attribute_Demon_Jingshen,jingshen - 5)
	...
	s_sysInfo(uid,string.format("使用成功，当前宠物化抗属性还原为%d点",jingshen-5))
	return VALUE_OK
end
