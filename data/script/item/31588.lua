--神兵武器宝箱
function OnUseItemEvent_31588(uid,thisid)
	local level = s_getValue(SCENE_ENTRY_PLAYER,uid,VALUE_TYPE_LEVEL)
	if level < 131 then
		s_sysInfo(uid,"等级不符，无法使用")
		return VALUE_FAIL
	end
	if s_deleteItemByThisID(uid,thisid,1,"OnUseItemEvent_31588") ~= VALUE_OK then
		return VALUE_FAIL
	end
	if level >= 91 and level <= 100 then
		local targetid = {54420,54421,54422,54423}
		local itemid = s_addItem(uid,targetid[math.random(1,table.getn(targetid))],3,1,0,"7-4","OnUseItemEvent_31588")
